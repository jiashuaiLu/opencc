...
export const ThemeProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const setMode = (newMode: ThemeMode) => {
    ...
    // Auto-switch glassLevel: dark→heavy, light→light
    const recommendedGlass = newMode === 'dark' ? 'heavy' : 'light';
    const updatedColors = { ...customColors, glassLevel: recommendedGlass as any };
    setCustomColorsState(updatedColors);
    saveSettings(newMode, updatedColors);
    message.info(`已自动切换毛玻璃强度为「${newMode === 'dark' ? '浓郁' : '清透'}」`);
  };
